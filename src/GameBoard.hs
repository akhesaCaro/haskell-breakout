...
foldBrick ::
     GameLevel -- ^ level number (probability to have a brick)
...
foldBrick level pos (sgItem, sgBrick, bricks) =
  case isBrick of
    True ->
      case isItem of
        True ->
          (newSGItem', newSGBrick, Brick yellow (Just itemType) pos : bricks)
